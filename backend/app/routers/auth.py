...
def student_login(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    db.add(ActivityLog(student_id=user.id, event_type=ActivityEventType.login, event_metadata={}))
    db.commit()
...
def logout(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.add(ActivityLog(student_id=user.id if user.role == UserRole.student else None, event_type=ActivityEventType.logout))
    db.commit()
